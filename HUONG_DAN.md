...
10. **Lưu ý về file `.npmrc` và lỗi build Cloudflare (ERESOLVE)**
    - Do dự án có sử dụng thư viện `@codetrix-studio/capacitor-google-auth` (yêu cầu bộ nguồn Capacitor v6) nhưng lại cài đặt Capacitor v8 mới nhất, tiến trình tự động `npm clean-install` của Cloudflare Pages sẽ báo lỗi xung đột phiên bản `ERESOLVE` và từ chối build.
    - **Cách giải quyết:** Đã có sẵn tệp `.npmrc` ở thư mục gốc chứa cấu hình `legacy-peer-deps=true`. Tệp này bắt buộc **phải được giữ lại và push lên GitHub** để yêu cầu Cloudflare bỏ qua lỗi xung đột khi kiểm tra thư viện! Vui lòng không xóa tệp này.
